...
export default function RegisterPage() {
  ...
  const [bloodBanks, setBloodBanks] = useState<Array<{id: string, name: string, city: string}>>([]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.role === "BLOOD_BANK" && !formData.bloodBankId && !formData.bankName) {
      setError("Please select an existing blood bank or enter new bank details");
      return;
    }
    if (formData.role === "HOSPITAL" && !formData.hospitalName) {
      setError("Please enter your hospital name or select an existing hospital");
      return;
    }
  ...
  const checkField = async (field: "email" | "phone", value: string) => {
    try {
      ...
      if (field === "email") {
        setFieldErrors((s) => ({ ...s, email: data.existsEmail ? "Email already registered" : undefined }));
      } else {
        setFieldErrors((s) => ({ ...s, phone: data.existsPhone ? "Phone number already registered" : undefined }));
      }
    } catch (e) {
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
              {formData.role === "BLOOD_BANK" ? (
                <>
                  ...
                  <div>
                    <label htmlFor="bankPhone" className="block text-sm font-medium text-gray-700">Contact Phone *</label>
                    <input
                      id="bankPhone"
                      name="bankPhone"
                      type="text"
                      ...
                      value={formData.bankPhone}
                          ...
                          onBlur={() => {
                            ...
                            if (!validatePhoneFormat(v)) {
                              setFieldErrors((s) => ({ ...s, phone: 'Phone must be exactly 10 digits' }));
                            } else {
                ...
                <>
                  ...
                  <div>
                    <label htmlFor="hospitalPhone" className="block text-sm font-medium text-gray-700">Contact Phone *</label>
                    <input
                      id="hospitalPhone"
                      name="hospitalPhone"
                      type="text"
